...
<template>
    <DropdownCustom class="app-header-message-vue" placement="bottom-end" className="app-message-dropdown"
                    :toggleIcon="false" ref="messageDropdown">
            ...
            <div class="h-panel">
                ...
                <div class="message-list-container common-list-container">
                    <div class="common-list-item" v-for="m of messageList" :key="m.id" @click="goMessageDetail(m)"
                         :class="{readed: m.status==0, unReaded: !(m.status==0)}">
...
<script>
    ...
    import {queryMessage} from '../../../js/request/messageReq.js'
            ...
            getMessageList: async function () {
                ...
                queryMessage({}).then(res => {
                    if (res.data.errorCode == "0000") {
                        _this.messageList = res.data.body
                    }
                }).catch(res => {
                    _this.$Notice["error"]('消息请求异常')
                })
            },
            goMessageDetail() {
